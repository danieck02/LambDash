...
function broadcast(data) {
  clients.forEach((client) => {
    try { client.send(JSON.stringify(data)); } catch(err) {}
  });
}
...
module.exports = function auth(roleRequired = null) {
  return (req, res, next) => {
    ...
    try {
      ...
      req.user = decoded;
      next();
    } catch(err) { res.status(401).json({ error: "Token ongeldig" }); }
  };
};
EOF
...
const router = express.Router();
const auth = require("../middleware/authMiddleware");
router.get("/chauffeurs", auth(), (req,res)=>res.json([{naam:"Jan",ritten:12,status:"Actief"},{naam:"Piet",ritten:9,status:"In Rust"}]));
router.get("/routes", auth(), (req,res)=>res.json([{routeId:"R001",stops:14,chauffeur:"Jan"},{routeId:"R002",stops:8,chauffeur:"Piet"}]));
router.get("/voertuigen", auth("admin"), (req,res)=>res.json([{id:1,type:"Bakwagen",status:"Onderweg"},{id:2,type:"Trailer",status:"Garage"}]));
module.exports = router;
EOF

# Data
cat << 'EOF' > data/orders.json
{"nieuw":[],"oud":[],"verzonden":[],"aangekomen":[]}
EOF
